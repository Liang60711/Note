...
4. 表鎖的衝突表(這裡不討論行鎖)，`X` 和 `S` 分別代表 `表級寫鎖`、`表級讀鎖`
    ...
    1. `表級寫鎖(X)`表示獨佔整個表，因此與任何鎖都衝突。
    2. IX 和 IX 只要表示不同行，就不會衝突，IS 與 IX 同理，不同行就不衝突。
    3. S 和 IS 表示共享鎖，彼此皆不會衝突。
    4. S 和 IX 衝突原因是，此表已經有S鎖，就不能再加上行級 X 鎖。
